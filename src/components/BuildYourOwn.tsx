...
const rails: RailCard[] = [
  {
    ...
    labels: {
      ...
      ja: {
        ...
        headline: 'ラップランドでの宿泊',
        body:
          'ロヴァニエミのホテルから、サーリセルカのグラスイグルーまで。Trip.comで比較し、仲介者なしで直接ご予約いただけます。',
...
},
  {
    ...
    labels: {
      ...
      ko: {
        ...
        headline: '공항에서 렌터카',
        body:
          '로바니에미, 키틸래, 이발로에서 시작하는 자유 운전. 스파이크 겨울 타이어 기본 장착. 픽업 시 결제, 무료 변경.',
...
const COPY: Record<CopyLang, { eyebrow: string; h2: string; lead: string; affiliateNote: string }> = {
  ...
  ko: {
    ...
    h2: '직접 조립하기',
    lead:
      '숙박, 이동 수단, 현지에서 할 활동을 직접 선택하세요. 세 갈래(숙박·이동·액티비티), 실시간 가격, 각 파트너 사이트에서 직접 예약합니다.',
